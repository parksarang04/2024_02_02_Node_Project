...
app.post('/login',async(req,res)=>{
    ...
    try{
        ...
        if(players.lenth>0)
        {
            ...
            res.status(401).json({success: false,message : '로그린 실패'});
        }
    }
    catch (error)
    {
...
app.get('/inventory/:playerId',async(req, res)=>{
    try 
    {
        const [inventory] = await pool.query(
            'SELECT i.*inv.quantity FROM invrntories inv JOIN items i ON inv.item_id = i.item WHERE inv.player_id = ?',
            [req.params.playerId]
        );
        res.json(inventory);
    }
...
//퀘스트 목록 조회
app.get('/puests/:playerId',async(req,res)=>{
    try 
    {
        const [inventory] = await pool.query(
            'SELECT q.*,pq.status FROM player_quests pq JOIN quest q ON pq.quest_id = q.quest_id WHERE pq.player_id = ?',
            [req.params.playerId]
        );
        res.json(quest);
    }
...
app.post('/inventory/add',async(req,res)=>{
    ...
    try{
        ...
        `SELECT * FORM inventoryies WHERE player_id = ? AND item_id = ?`,
        [playerId,itemId]
        );
        ...
        {
            await pool.query(
                `UPDATE inventoryies SET quantity = quantity + ? WHERE player_id =? AND item_id = ?`,
